...
<div class="page-header"><h2> {{ $pageTitle }} <small> {{ $pageNote }} </small> </h2></div>
	...
	{!! Form::open(array('url'=>'mydesignation?return='.$return, 'class'=>'form-horizontal validated sximo-form','files' => true ,'id'=> 'FormTable' )) !!}
	...
	<div class="p-5">
		...
		<div class="row">
	<div class="col-md-12">
						<fieldset><legend> My Designation</legend>
   ...
   <script type="text/javascript">
	$(document).ready(function() { 
		...
		$("#status").jCombo("{!! url('mydesignation/comboselect?filter=status:id:title') !!}",
		{  selected_value : '{{ $row["status"] }}' });
		 	
		 	 

		$('.removeMultiFiles').on('click',function(){
			var removeUrl = '{{ url("mydesignation/removefiles?file=")}}'+$(this).attr('url');
			$(this).parent().remove();
